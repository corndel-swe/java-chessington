...
public abstract class AbstractPiece implements Piece {
  ...
  protected Stream<Move> getLateralMoves(Coordinates from, Board board) {
    ...
    return Stream.of(leftMoves, rightMoves, upMoves, downMoves)
        .flatMap(s -> s)
  ...
  protected Stream<Move> getDiagonalMoves(Coordinates from, Board board) {
    ...
    return Stream.of(upLeftMoves, upRightMoves, downLeftMoves, downRightMoves)
        .flatMap(s -> s)
        .map(c -> new Move(from, c));
  }

  private Stream<Coordinates> getValidTargetsInDirection(
      Coordinates origin, Function<Coordinates, Coordinates> nextCoord, Board board) {
